...
class ViewModel {
    ...
    func didTap(button: CalcButton) {
        value = CalculatorLogic.shared.handleTap(button: button, currentValue: value, lastToken: lastToken, modelContext: modelContext)
        lastToken = button
    }
}
...
class CalculatorLogic {
    ...
    func handleTap(button: CalcButton, currentValue: String, lastToken: CalcButton?, modelContext: ModelContext) -> String {
        ...
        case .equal:
            if lastToken != .equal {
                let result = evaluateExpression(expression: newValue)
                if !result.hasPrefix("Error") {
                    modelContext.insert(History(expression: newValue, result: result))
                    newValue = result
                }
            }
        case .negative:
